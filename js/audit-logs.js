...
import { db, collection, doc, getDoc, query, where, getDocs, orderBy, limit } from "./firebase-config.js";
...
async function loadUserProfiles() {
    ...
    await Promise.all(userIds.map(async (userId) => {
        try {
            ...
            userProfiles.set(userId, userSnapshot.exists() ? userSnapshot.data() : {});
        } catch (error) {
            debugError(`Unable to load audit user ${userId}:`, error);
...
function getUserLabel(userId) {
    ...
    return profile.displayName || profile.name || profile.email || userId || "";
}
    ...
    tbody.innerHTML = logs.map((log) => {
        ...
        return `
            <tr>
                ...
                <td>${escapeHtml(getUserLabel(log.userId))}</td>
...
function applyFilters() {
    ...
    const filtered = currentLogs.filter((log) => {
        if (currentFilters.search) {
            ...
            const user = getUserLabel(log.userId).toLowerCase();
            if (!action.includes(currentFilters.search) && !module.includes(currentFilters.search) && !user.includes(currentFilters.search)) return false;
        }
