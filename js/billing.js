...
window.printInvoice = async function() {
    ...
    try {
        ...
        // The OPD visit document only carries a doctorName when the visit was
        // created through the OPD walk-in check-in form. Visits started from
        // patient registration/profile, and appointments, are created without
        // it, and the consultation flow never writes the name back to the OPD
        // doc (it stores it on encounters/diagnoses instead). So fall back to
        // the encounter/diagnosis records for the visit so the invoice's
        // "Attending Doctor" isn't left blank on insurance claims.
        let attendingDoctor = selectedVisit.doctorName || "";
        if (!attendingDoctor && isInsurance) {
            try {
                ...
                if (!encSnap.empty) {
                    attendingDoctor = encSnap.docs[0].data().doctorName || "";
                }
                if (!attendingDoctor) {
                    ...
                    if (!diagSnap.empty) {
                        attendingDoctor = diagSnap.docs[0].data().doctorName || "";
                    }
                }
            } catch (error) {
                debugError("Error resolving attending doctor for invoice:", error);
            }
        }
            ...
            <body>
                ...
                <div class="meta">
                    <div class="left">
                        ...
                        ${isInsurance ? `<p><strong>Attending Doctor:</strong> ${escapeHtml(attendingDoctor || "—")}</p>` : ""}
