/**
 * PRINCE ALEX DIGITAL HMS — SVG Icon System
 * 
 * Provides a centralized set of inline SVG icons to replace emoji icons
 * across the entire application. This ensures a professional, consistent
 * look and removes all emoji characters from the UI.
 */
 ...
 * @param {string} name - The icon name (e.g. "dashboard", "patients")
 * @param {string} size - The size in px (default "20")
...
export function icon(name, size = "20", className = "") {
    ...
    return `<svg xmlns="${SVG_NS}" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"${cls}>${paths}</svg>`;
}

/**
 * Icon path definitions (Feather-style Icons).
 * Each entry is the inner SVG content.
 */
const ICON_PATHS = {
    ...
    register: '<path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/>',
    ...
    'new-appointment': '<path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/>',
    opd: '<path d="M3 21h18"/><path d="M5 21V7l8-4v18"/><path d="M19 21V11l-6-4"/>',
    ...
    'new-admission': '<path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/>',
    ...
    'lab-results': '<line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/>',
    ...
    'stock-movements': '<polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/>',
...
/**
 * Replaces all emoji icons in a container with SVG icons.
 * @param {HTMLElement} container - The container to scan
 */
export function replaceEmojisWithIcons(container = document) {
    const emojiMap = {
        "📊": "dashboard",
        "👥": "patients",
        "👨‍⚕️": "staff",
    ...
    nodes.forEach((node) => {
        ...
        for (const [emoji, iconName] of Object.entries(emojiMap)) {
            html = html.split(emoji).join(icon(iconName, "18", "icon-svg"));
        }
