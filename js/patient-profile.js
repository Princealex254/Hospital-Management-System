...
async function loadPatientVitals() {
    ...
    try {
        ...
        if (snapshot.empty) {
            ...
            const vitals = snapshot.docs.map(vitalDoc => ({ id: vitalDoc.id, ...vitalDoc.data() }));
            const userIds = [...new Set(vitals
                .filter(vital => !vital.recordedByName && vital.recordedBy)
                .map(vital => vital.recordedBy))];
            const recordedByNames = new Map();
            ...
            await Promise.all(userIds.map(async (userId) => {
                try {
                    ...
                    if (userSnapshot.exists()) {
                        ...
                        recordedByNames.set(userId, user.displayName || user.name || user.email || userId);
                    }
                } catch (error) {
                    debugError("Unable to resolve vital recorder:", error);
                }
            }));
            ...
            vitals.forEach((v) => {
                const recordedBy = v.recordedByName || recordedByNames.get(v.recordedBy) || v.recordedBy || "";
                html += `<tr>
...
function setupEditButton() {
    ...
    if (editBtn) {
        editBtn.addEventListener("click", () => {
            if (!currentPatient) return;
            window.location.href = `../patient-profile/?id=${currentPatient.id}&edit=true`;
        });
    }
}
...
async function startPatientVisit(patientDocId) {
    ...
    try {
        ...
        setTimeout(() => {
            window.location.href = `../queue/?visitId=${visitRef.id}`;
        }, 1000);
