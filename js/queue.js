...
<td class="text-right">
                    <div class="table-actions">
                        ...
                        ${status === "SERVICES_PENDING" || status === "BILLING_PENDING" || status === "PAYMENT_PENDING" || status === "READY_FOR_CHECKOUT"
                            ? `<a href="../patient-profile/?id=${apt.patientId}" class="btn btn-sm btn-outline">${icon('eye', '18')} View Visit</a>`
                            : ""
                        }
...
window.startTriage = async function(visitId) {
    ...
    try {
        ...
        window.location.href = `../triage/?visitId=${visitId}`;
    } catch (error) {
...
window.startConsultation = async function(visitId) {
    ...
    try {
        ...
        window.location.href = `../consultation/?visitId=${visitId}`;
    } catch (error) {
