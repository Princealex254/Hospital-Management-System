...
import { getCurrentUser, hasPermission, hasAreaAccess, hasRole, ROLES, setCurrentUser } from "./permissions.js";
    ...
    const sidebarHtml = `
        <div class="sidebar" id="sidebar">
            ...
            <nav class="sidebar-nav">
                <ul>
                    <li>
                        <a href="../dashboard/" class="nav-item ${isCurrentPage('../dashboard/') ? 'active' : ''}">
...
function setupSidebarNavHighlighting() {
    ...
    navLinks.forEach((link) => {
        ...
        const linkSlug = href.replace(/^\.\.\//, "").replace(/\/+$/, "").split(/[?#]/)[0];
        if (linkSlug === currentFolder) {
...
function isCurrentPage(href) {
    const currentFolder = window.location.pathname.replace(/\/+$/, "").split("/").filter(Boolean).pop() || "";
    const linkSlug = (href || "").replace(/^\.\.\//, "").replace(/\/+$/, "").split(/[?#]/)[0];
    return linkSlug !== "" && linkSlug === currentFolder;
}
