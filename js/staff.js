...
import { getTenantId, getCurrentUser, hasPermission, PERMISSIONS, ROLE_PERMISSIONS, ACCESS_AREAS } from "./permissions.js";
                ...
                <td class="text-right">
                    <div class="table-actions">
                        <button class="btn btn-sm btn-outline" onclick="editStaff('${member.id}')"> ${icon('edit', '18', 'icon-svg')} Edit</button>
                            <button class="btn btn-sm btn-error" onclick="deleteStaff('${member.id}', '${escapeHtml(member.displayName || "")}')"> ${icon('trash', '18', 'icon-svg')} Delete</button>
                    </div>
                </td>
            </tr>
        `;
...
function renderAccessAreaOptions(selectedAreas = Object.values(ACCESS_AREAS), inputName = "staff-access-area") {
    return Object.values(ACCESS_AREAS).map(area => `
        <label class="form-check">
            <input type="checkbox" name="${inputName}" value="${area}" ${selectedAreas.includes(area) ? "checked" : ""}>
            <span>${accessAreaLabels[area]}</span>
        </label>
    `).join("");
}

function getSelectedAccessAreas(inputName) {
        ...
        <div class="modal" style="max-width: 600px;">
            ...
            <div class="modal-body">
                <div class="form-grid form-grid-2">
                    ...
                    <div class="form-group full-width"><label class="form-label">Access Areas</label><div class="form-check-grid">${renderAccessAreaOptions()}</div></div>
...
window.saveStaff = async function() {
    ...
    try {
        ...
        await setDoc(doc(db, "users", staffUid), {
            ...
            accessAreas: getSelectedAccessAreas("staff-access-area"),
            phone: phone || null,
    ...
    const modalHtml = `
        <div class="modal" style="max-width: 600px;">
            <div class="modal-header"><h3>Edit Staff Member</h3><button class="modal-close" data-modal-close>&times;</button></div><div class="modal-body">
                <div class="form-grid form-grid-2">
                    <div class="form-group"><label class="form-label" for="edit-staff-name">Name</label><input type="text" id="edit-staff-name" class="form-input" value="${escapeHtml(member.displayName || "")}"></div>
                    <div class="form-group"><label class="form-label" for="edit-staff-role">Role</label>
                        <select id="edit-staff-role" class="form-select" disabled>
                            <option value="${escapeHtml(member.role || "")}" selected>${escapeHtml(formatRole(member.role))}</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group full-width"><label class="form-label">Access Areas</label><div class="form-check-grid">${renderAccessAreaOptions(Array.isArray(member.accessAreas) ? member.accessAreas : Object.values(ACCESS_AREAS), "edit-staff-access-area")}</div></div>
                    ...
                    <div class="form-group"><label class="form-label" for="edit-staff-email">Email</label><input type="email" id="edit-staff-email" class="form-input" value="${escapeHtml(member.email || "")}" readonly></div>
                    <div class="form-group"><label class="form-label" for="edit-staff-status">Status</label>
                        <select id="edit-staff-status" class="form-select">
                            <option value="active" ${(member.accountStatus || "active") === "active" ? "selected" : ""}>Active</option>
                            <option value="inactive" ${member.accountStatus === "inactive" ? "selected" : ""}>Inactive</option>
                        </select>
                    </div>
                </div>
            </div>
...
window.updateStaff = async function(staffId) {
    ...
    try {
        ...
        await updateDoc(doc(db, "users", staffId), {
            displayName: document.getElementById("edit-staff-name").value.trim(),
            ...
            accountStatus: document.getElementById("edit-staff-status").value,
            accessAreas: getSelectedAccessAreas("edit-staff-access-area"),
            updatedAt: serverTimestamp()
        });
        await addDoc(collection(db, "auditLogs"), {
